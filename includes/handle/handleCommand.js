    ...
    return async function (event) {
        ...
        if (event.senderID != api.getCurrentUserID() && !ADMINBOT.includes(senderID)) {
            ...
            if ((prefixTO[threadID] + 'bank') != event.body[0]) {
                ...
                if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now() + 25200000) return api.sendMessage(`[  Thông Báo Gia Hạn  ]\n-----------------------\n⚠️ Thời hạn sử dụng bot của nhóm bạn đã hết. Vui lòng reply tin nhắn này và nhập mã key mới\n hoặc liên hệ Admin để được hỗ trợ.\nfb: ${(!global.config.FACEBOOK_ADMIN) ? "Exclude Admin if not configured!" : global.config.FACEBOOK_ADMIN}`, event.threadID, (e, i) => {
                    global.client.handleReply.push({
                        name: 'rent',
                        messageID: i.messageID,
                        threadID: event.threadID,
        ...
        if (!ADMINBOT.includes(senderID) && MAINTENANCE) {
            return api.sendMessage('⚠️ ANH KIÊN ĐÉO CHO MÀY DÙNG BOT HAHA', threadID, messageID);
        }
